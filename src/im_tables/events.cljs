...
(reg-event-fx
  ...
  (let [fetch-atom (r/atom (chan))]
    (fn [{db :db} [_ loc service model query lang]]
      ; Close any previous code generation requests
      (swap! fetch-atom close!)
      (let [new-request (fetch/code service model {:query query :lang lang})]
        ; Store the new request
        (reset! fetch-atom new-request)
        {:db                          (-> db
                                          (assoc-in [:settings :codegen :lang] lang)
                                          (assoc-in [:codegen :code] nil))
         :im-tables/im-operation-chan {:on-success [:main/save-codegen loc lang]
                                       :channel    new-request}}))))
