...
(reg-event-fx
  :main/generate-code
  (sandbox)
  (fn [{db :db} [_ loc service model query]]
    {:db db
     :im-tables/im-operation-chan {:on-success [:main/save-codegen loc service query]
                                   :channel (fetch/code service model {:query query :lang "js"})}}))

(defn generate-javascript [{:keys [cdn query service]}]
  (string/join "\n"
               [
                "<script>"
...
(reg-event-db
  :main/save-codegen
  (sandbox)
  (fn [db [_ loc service query response]]
    (js/console.log (generate-javascript {:service service
                                          :query query}))
    (assoc-in db [:codegen :code] response)))